export const zhTW = {
  common: {
    appName: '王國建造者',
    pointsShort: '分',
    player: '玩家 {{number}}',
    language: '語言',
    english: 'English',
    traditionalChinese: '繁體中文',
  },
  phase: {
    Setup: '設定',
    DrawCard: '抽牌',
    PlaceSettlements: '放置聚落',
    EndTurn: '結束回合',
    GameOver: '遊戲結束',
  },
  terrain: {
    Grass: '草原',
    Forest: '森林',
    Desert: '沙漠',
    Flower: '花田',
    Canyon: '峽谷',
    Water: '水域',
    Mountain: '山脈',
  },
  location: {
    Castle: '城堡',
    Farm: '農場',
    Oasis: '綠洲',
    Tower: '高塔',
    Harbor: '港口',
    Paddock: '牧場',
    Barn: '穀倉',
    Oracle: '神諭',
    Tavern: '酒館',
  },
  locationDescription: {
    Castle: '相鄰城堡的每個己方聚落得 3 分（固定計分，一律套用）。',
    Farm: '在草原放置 1 個額外聚落（優先相鄰自己聚落的格）。',
    Harbor: '在任意空的可建造且相鄰水域的格子放置 1 個額外聚落。',
    Oasis: '在沙漠放置 1 個額外聚落（優先相鄰自己聚落的格）。',
    Tower: '在棋盤邊緣格放置 1 個額外聚落（優先相鄰自己聚落的格）。',
    Paddock: '將你的 1 個聚落移動到 2 格內空的可建造格。',
    Barn: '將你的 1 個聚落移動到任意空的相同地形格。',
    Oracle: '在當前地形卡所示地形的格放置 1 個額外聚落（優先相鄰自己聚落的格）。',
    Tavern: '在你的水平連續聚落列（至少 3 個）的尾端放置 1 個額外聚落。',
  },
  tooltip: {
    tapToDismiss: '點一下關閉',
  },
  objective: {
    Fisherman: '漁夫',
    Miners: '礦工',
    Knights: '騎士',
    Farmers: '農夫',
    Merchants: '商人',
    Discoverers: '發現者',
    Hermits: '隱士',
    Citizens: '市民',
    Lords: '領主',
    Workers: '工人',
  },
  objectiveDescription: {
    Fisherman: '每個相鄰水域的相連聚落群得 2 分。',
    Miners: '每個相鄰山脈的聚落得 2 分。',
    Knights: '你最長的水平連續聚落鏈，每個聚落得 1 分。',
    Farmers: '你聚落最多的棋盤象限，該象限每個聚落得 1 分。',
    Merchants: '你的聚落占據的每種不同地形各得 1 分。',
    Discoverers: '你的聚落占據的每一列（橫列）各得 1 分。',
    Hermits: '每個沒有相鄰友方聚落的孤立聚落得 3 分。',
    Citizens: '每個相鄰城堡的聚落得 3 分。',
    Lords: '每個至少有一個己方聚落的象限得 3 分。',
    Workers: '每個相鄰地點格（包含城堡）的聚落各得 1 分。',
  },
  objectiveCard: {
    currentScore: '{{score}} 分',
  },
  menu: {
    subtitle: '策略性王國建造遊戲',
    singlePlayer: '單人模式',
    onlineMultiplayer: '線上多人對戰',
    continue: '繼續遊戲',
    noSave: '無存檔',
    tutorial: '教學',
    leaderboard: '排行榜',
    achievements: '成就',
    replayLibrary: '回放記錄',
    settings: '設定',
    back: '返回',
    otherGames: '更多遊戲',
    herbalism: '本草棋遊',
    herbalismDesc: '中草藥桌遊',
    evolution: '演化論',
    evolutionDesc: '生物演化桌遊',
    sudoku: '數獨',
    sudokuDesc: '數字謎題',
  },
  settings: {
    heading: '設定',
    close: '關閉設定',
    audio: '音訊',
    mute: '靜音',
    muted: '已靜音',
    unmuted: '開啟',
    volume: '音量',
    volumeValue: '{{value}}%',
    language: '語言',
  },
  multiplayer: {
    serverUrl: '伺服器網址',
    yourName: '你的名稱',
    playerNamePlaceholder: '玩家名稱',
    roomCodePlaceholder: '房間代碼',
    connect: '連線',
    connected: '已連線',
    reconnecting: '重新連線中...',
    createRoom: '建立房間',
    join: '加入',
    room: '房間',
    host: '房主',
    you: '你',
    disconnected: '已斷線',
    ready: '準備就緒',
    notReady: '尚未準備',
    setReady: '設為準備',
    setNotReady: '取消準備',
    obj: '目標',
    undo: '復原',
    enabled: '啟用',
    disabled: '停用',
    startGame: '開始多人遊戲',
    leaveRoom: '離開房間',
  },
  app: {
    liveAnnouncement: '{{player}}回合 — {{phase}}',
    liveAnnouncementWithTerrain:
      '{{player}}回合 — {{phase}}，地形：{{terrain}}，剩餘放置：{{count}}',
    playOnlineMultiplayer: '🌐 線上多人模式',
    roomStatus: '房間 {{id}} · {{status}}',
    mute: '靜音',
    unmute: '取消靜音',
    gameInformation: '遊戲資訊',
    currentPlayer: '目前玩家',
    currentPlayerRegion: '目前玩家：{{name}}',
    botDifficultyEasy: '簡單',
    botDifficultyMedium: '中等',
    botDifficultyHard: '困難',
    settlementsRemaining: '剩餘聚落：{{count}}',
    phase: '階段',
    gamePhaseRegion: '遊戲階段',
    currentTerrainCardRegion: '目前地形卡',
    terrain: '地形',
    placementsLeft: '剩餘放置：{{count}}',
    objectives: '目標卡',
    yourTiles: '你的地點牌',
    cancel: '取消',
    use: '使用',
    used: '已用',
    clickHighlightedDestinationToMove: '點選目的地格子完成移動',
    clickHighlightedSettlementToMove: '點選要移動的你的聚落（亮色格）',
    clickHighlightedCellToPlace: '點擊高亮格子以放置。',
    extraPlacementNote: '（{{tileName}}額外放置，不計入本回合 3 個配額）',
    actions: '操作',
    drawTerrainCardAria: '抽地形卡開始回合',
    drawTerrainCard: '抽地形卡',
    placementHelp:
      '點擊或按 Enter 於高亮六角格放置聚落。可用方向鍵移動焦點，按 Escape 取消選取。',
    placementsRemaining: '剩餘 {{count}} 次放置',
    undo: '復原',
    endTurnAria: '結束回合並交給下一位玩家',
    endTurn: '結束回合',
    liveScores: '即時計分',
    players: '玩家',
    current: '目前',
    placedRemainingSummary: '已放置：{{placed}} | 剩餘：{{remaining}}',
    tilesSummary: '地點牌：{{tiles}}',
    tilesLabel: '地點牌',
    playerListAria:
      '{{name}}：已放置 {{placed}}，剩餘 {{remaining}}{{isCurrent}}',
    playerListAriaCurrentSuffix: '，目前玩家',
    castleScoreSummary: '{{castle}} | {{objectives}}',
    objectiveScoreItem: '{{card}}：{{score}}',
  },
  setup: {
    gameSetup: '遊戲設定',
    numberOfPlayers: '玩家人數',
    playerLabel: '玩家 {{number}}',
    playerPlaceholder: '玩家 {{number}}',
    human: '玩家',
    computer: '電腦',
    aiDifficulty: 'AI 難度',
    difficultyEasy: '簡單（隨機）',
    difficultyMedium: '中等（策略）',
    difficultyHard: '困難（Alpha-Beta）',
    difficultyLegacy: '中等（舊版）',
    boardSize: '地圖大小',
    boardSizeSmall: '小型 (12×12)',
    boardSizeMedium: '中型 (16×16)',
    boardSizeLarge: '大型 (20×20)',
    gameOptions: '遊戲選項',
    objectiveCards: '目標卡數量',
    allowUndo: '允許復原（每回合一次）',
    tutorial: '📖 教學：如何遊玩',
    startGame: '開始遊戲',
    useCustomMap: '使用自訂地圖',
    customMapFromStore: '從已存地圖選擇',
    customMapFromCode: '貼入分享碼',
    noCustomMaps: '尚無已存地圖，請先至地圖編輯器建立',
    invalidShareCode: '分享碼無效，請確認後重試',
    mapNotFound: '地圖不存在，請重新選擇',
    customMapMultiplayerNote: '多人模式不支援自訂地圖',
  },
  gameOver: {
    heading: '遊戲結束！',
    finalRankings: '最終排名',
    castleScore: '城堡：{{score}} 分',
    objectiveScore: '{{card}}：{{score}} 分',
    castleSegment: '城堡',
    segmentScore: '{{label}}：{{score}} 分',
    scoreBarAriaLabel: '{{player}} 得分分解，總分 {{score}} 分',
    scoreSegmentAriaLabel: '{{player}} {{label}} 得分：{{score}} 分',
    objectiveCards: '目標卡',
    newGame: '新遊戲',
    winnerAnnounce: '{{name}} 獲勝！',
    tieAnnounce: '平手！',
  },
  leaderboard: {
    open: '排行榜',
    heading: '排行榜',
    close: '關閉',
    localTab: '本機',
    globalTab: '全球',
    empty: '目前沒有排行榜紀錄。',
    clearLocal: '清除本機紀錄',
    filterByObjective: '目標卡組合',
    filterAll: '全部',
    meta: '{{date}} · {{playerCount}} 位玩家',
  },
  replay: {
    open: '回放紀錄',
    heading: '遊戲回放',
    close: '關閉',
    empty: '尚無回放紀錄，完成一局遊戲即可儲存回放！',
    watchReplay: '觀看回放',
    backToList: '← 回到列表',
    stepOf: '步驟 {{current}} / {{total}}',
    previous: '← 上一步',
    next: '下一步 →',
    winner: '勝者：{{name}}',
    players: '玩家',
    date: '日期',
    viewReplay: '觀看回放',
    deleteReplay: '刪除',
    noActions: '沒有任何操作紀錄。',
    actionTurn: '回合 {{turn}}',
    actionPlayer: '{{player}}',
    actionPlaceSettlement: '在 Q{{q}}R{{r}} 放置聚落',
    actionUseTile: '在 Q{{q}}R{{r}} 使用 {{tile}}',
    actionMoveSettlement: '移動聚落 Q{{fromQ}}R{{fromR}} → Q{{toQ}}R{{toR}}',
    actionUnknown: '操作',
    finalScores: '最終得分',
    replayOf: '回放 — {{date}}',
    actionCount: '{{count}} 個操作',
    actionCount_plural: '{{count}} 個操作',
  },
  gameLog: {
    heading: '操作日誌',
    empty: '尚無操作記錄',
    turn: '[回合 {{turn}}] {{player}}',
    placeSettlement: '放置聚落',
    placeSettlementAt: '放置聚落於 Q{{q}}R{{r}}',
    useTile: '使用 {{tile}}',
    useTileAt: '使用 {{tile}} 於 Q{{q}}R{{r}}',
    moveSettlement: '移動聚落',
    moveSettlementFromTo: '移動聚落 Q{{fromQ}}R{{fromR}} → Q{{toQ}}R{{toR}}',
    action: '操作',
    acquiredTile: '（獲得 {{emoji}} {{tile}}）',
    last: '最近：',
    lastSettlementAt: '於 Q{{q}}R{{r}} 放置聚落',
    lastMovedSettlement: '移動聚落',
    lastUsedTile: '使用地點牌',
  },
  turnBanner: {
    regionLabel: '{{player}}的回合 — {{phase}}',
    playerTurn: '{{player}} 的回合',
    phaseLabel: '階段',
    placementsProgress: '已放置 {{placed}} / {{total}} 個聚落',
    ofSettlements: '{{placed}} / {{total}}',
    noValidPlacements: '無可放置位置',
    botThinking: '對手行動中…',
  },
  sidebar: {
    terrainCard: '地形卡',
    yourTiles: '你的地點板塊',
    objectives: '目標卡',
    liveScores: '即時計分',
    players: '玩家',
    collapseSidebar: '收合側欄',
    expandSidebar: '展開側欄',
  },
  bottomDrawer: {
    controlsRegion: '遊戲控制',
    closePanel: '收合遊戲面板',
    openPanel: '展開遊戲面板',
    tapHighlightedHex: '點擊高亮六角格以放置聚落',
    placementsLeft: '剩餘 {{count}} 次放置',
    tapHighlightedDestinationToMove: '點擊高亮目的地以移動。',
    tapHighlightedSettlementToMove: '點擊高亮聚落以移動。',
    tapHighlightedCellToPlace: '點擊高亮格子以放置。',
    leading: '領先',
    mobileFloating: {
      draw: '抽牌',
      endTurn: '結束回合',
      undo: '復原',
    },
  },
  board: {
    gridLabel: '王國建造者棋盤',
    resetZoomTitle: '重設縮放',
    resetZoomAria: '重設棋盤縮放',
    settlementSuffix: '{{player}} 的聚落',
    validPlacementSuffix: '可放置',
  },
  hint: {
    invalid: {
      occupied: '此格已有聚落',
      terrain_unbuildable: '無法在{{terrain}}放置聚落',
      wrong_terrain: '本回合須放在{{terrain}}地形',
      must_be_adjacent: '須相鄰本回合已放的聚落',
      not_valid: '無法在此放置',
    },
  },
  installPrompt: {
    ariaLabel: '安裝到主畫面',
    button: '📲 安裝到主畫面',
  },
  saveLoad: {
    continueGame: '繼續遊戲',
    newGame: '新遊戲',
    clearSave: '清除存檔',
  },
  achievement: {
    panelLabel: '成就',
    heading: '成就',
    close: '關閉成就',
    progress: '已解鎖 {{unlocked}} / {{total}}',
    unlocked: '成就解鎖！',
    locked: '尚未解鎖',
    unlockedOn: '解鎖於 {{date}}',
    lockedAria: '{{title}} — 尚未解鎖',
    unlockedAria: '{{title}} — 已解鎖',
    open: '成就',
    items: {
      first_win: {
        title: '初次勝利',
        description: '贏得第一場遊戲。',
      },
      win_5: {
        title: '老手',
        description: '贏得 5 場遊戲。',
      },
      win_10: {
        title: '冠軍',
        description: '贏得 10 場遊戲。',
      },
      first_game: {
        title: '初學者',
        description: '完成第一場遊戲。',
      },
      play_5: {
        title: '常客',
        description: '完成 5 場遊戲。',
      },
      play_20: {
        title: '忠實玩家',
        description: '完成 20 場遊戲。',
      },
      score_50: {
        title: '得分高手',
        description: '單場得分達 50 分以上。',
      },
      score_100: {
        title: '百分王',
        description: '單場得分達 100 分以上。',
      },
      settlements_100: {
        title: '建造者',
        description: '累積放置 100 個聚落。',
      },
      settlements_500: {
        title: '建築師',
        description: '累積放置 500 個聚落。',
      },
      turns_50: {
        title: '耐心策略家',
        description: '累積進行 50 回合。',
      },
      tile_collector: {
        title: '地點大師',
        description: '終局時持有 3 張以上地點牌。',
      },
    },
  },
  season: {
    open: '賽季',
    currentSeason: '本賽季',
    heading: '賽季',
    seasonTab: '本賽季',
    historyTab: '賽季紀錄',
    label: '{{period}} 賽季',
    daysLeft: '剩餘 {{count}} 天',
    daysLeft_plural: '剩餘 {{count}} 天',
    empty: '本賽季尚無紀錄。',
    historyEmpty: '尚無歷史賽季。',
    myBestScore: '最高：{{score}} 分',
    myRank: '排名第 {{rank}}',
    rewardGold: '🥇 金獎',
    rewardSilver: '🥈 銀獎',
    rewardBronze: '🥉 銅獎',
    noReward: '—',
  },
  tutorial: {
    dialogLabel: '教學',
    close: '關閉教學',
    stepCounter: '步驟 {{current}} / {{total}}',
    back: '← 上一步',
    finish: '完成 🎉',
    next: '下一步 →',
    keyboardHint: '按左右方向鍵切換 · Esc 關閉',
    performHighlightedAction: '執行高亮操作即可自動前進。',
    onboardingPrompt: {
      title: '第一次玩嗎？',
      description: '要看快速教學了解遊戲規則嗎？',
      startButton: '開始教學',
      skipButton: '先跳過',
    },
    steps: {
      welcome: {
        title: '歡迎來到王國建造者！',
        description:
          '本教學會帶你快速了解遊戲核心規則。你將在六角格王國中建立聚落並取得高分。按「下一步」繼續。',
      },
      board: {
        title: '遊戲棋盤',
        description:
          '棋盤由不同地形六角格組成：草原🌿、森林🌲、沙漠🏜️、花田🌸、峽谷🪨、水域💧與不可通行山脈⛰️。你只能在可建造地形（非水域與山脈）放置聚落。',
      },
      castles: {
        title: '城堡與地點牌',
        description:
          '黃色城堡🏰是重要得分點，終局時鄰接城堡可獲得額外分數。特殊地點格（農場、綠洲、高塔等）在相鄰放置聚落時會給你強力地點牌。',
      },
      terrainCard: {
        title: '地形卡',
        description:
          '每回合開始你會抽一張地形卡，決定本回合 3 個聚落必須放在哪種地形。不能自行選擇，由卡片決定！按「抽地形卡」開始回合。',
      },
      placementRules: {
        title: '放置規則',
        description:
          '每回合必須放置剛好 3 個聚落於卡片指定地形。若你已有聚落且旁邊有合法地形，則必須鄰接既有聚落放置（鄰接規則）。高亮格即為合法位置。',
      },
      locationTileGain: {
        title: '獲得地點牌',
        description:
          '當你在地點格旁邊放置聚落時，會自動獲得該地點牌並加入收藏。地點牌每回合可使用一次，能觸發移動聚落等特殊能力。',
      },
      objectives: {
        title: '目標卡',
        description:
          '遊戲開始會揭示三張目標卡，描述不同計分條件（例如最大連通聚落群、靠近城堡等）。所有玩家都以相同目標計分。',
      },
      turnFlow: {
        title: '回合流程',
        description:
          '1️⃣ 抽地形卡 → 2️⃣ 在對應地形放置 3 個聚落（遵守鄰接規則）→ 3️⃣ 結束回合。可在結束前選擇使用地點牌能力，直到聚落放完為止。',
      },
      scoring: {
        title: '計分與勝利',
        description:
          '當所有玩家都放完 40 個聚落後遊戲結束。依城堡鄰接、目標卡與額外效果結算總分，最高分玩家獲勝！',
      },
      done: {
        title: '你已準備好開始！',
        description:
          '核心內容就到這裡。開始新遊戲試試看吧，祝你成為最強王國建造者！之後也可隨時從設定畫面重新開啟教學。',
      },
    },
  },
  quitConfirm: {
    title: '離開遊戲？',
    bodySolo: '你可以隨時從主選單繼續這局遊戲。',
    bodyMultiplayer: '你將離開房間，其他玩家將繼續遊戲。',
    confirm: '離開遊戲',
    cancel: '繼續遊戲',
  },
  newGameOverwrite: {
    title: '開始新遊戲？',
    body: '你有一局進行中的遊戲。開始新局將覆蓋現有存檔，舊局無法恢復。',
    confirm: '開始新局',
    cancel: '返回',
  },
  mapEditor: {
    title: '地圖編輯器',
    subtitle: '設計你的自訂地圖',
    backToMenu: '返回主選單',
    sectionTerrain: '地形',
    sectionLocation: '地點',
    clearAll: '清除全部',
    clearAllConfirm: '清除所有格子？此動作無法還原。',
    boardSize: '棋盤大小',
    boardSizeChangeConfirm: '更換尺寸將重設地圖，確定繼續？',
    saveMap: '儲存地圖',
    mapNamePlaceholder: '地圖名稱...',
    saveSuccess: '地圖已儲存！',
    maxMapsReached: '已達上限 10 張地圖，請先刪除一張。',
    exportCode: '分享碼',
    copyCode: '複製',
    copiedCode: '已複製！',
    openList: '我的地圖',
    importCode: '匯入分享碼',
    importPlaceholder: '貼上 KB-v1-... 分享碼',
    importBtn: '匯入',
    importFailed: '無效或損毀的分享碼。',
    importSuccess: '地圖匯入成功！',
    noSavedMaps: '尚無已儲存地圖。',
    loadMap: '載入',
    deleteMap: '刪除',
    entryButton: '地圖編輯器',
    editMode: '編輯模式',
    modePaint: '繪製',
    modePan: '平移',
  },
} as const;

  ...
  return (
    <>
      ...
      <div
        ...
        `}
        role="region"
        aria-label={t('drawer.gameControls')}
      ...
      >
        ...
        <div
          ...
          aria-label={isOpen ? t('drawer.closeGamePanel') : t('drawer.openGamePanel')}
          role="button"
          tabIndex={0}
        ...
        <div className="px-4 pb-6 space-y-3 max-h-[60vh] overflow-y-auto">
          ...
          {phase === GamePhase.PlaceSettlements && !activeTile && (
            <div className="p-3 bg-yellow-50 border border-yellow-300 rounded-xl text-sm text-center">
              {t('drawer.tapHighlightedHex')}
              <br />
              <span className="text-xs text-gray-500">
                {t('drawer.placementLeft', { count: remainingPlacements })}
              </span>
            </div>
          )}
          ...
          {activeTile && (
            <div className="p-3 bg-orange-50 border border-orange-300 rounded-xl text-sm text-center">
              {activeTile === Location.Paddock || activeTile === Location.Barn
                ? tileMoveFrom
                  ? t('drawer.tapHighlightedDestinationToMove')
                  : t('drawer.tapHighlightedSettlementToMove')
                : t('drawer.tapHighlightedCellToPlace')}
            </div>
          )}
          ...
          {lastAction && (
            <p className="text-xs text-gray-400 text-center">
              {t('drawer.lastActionPrefix')} {lastAction.type === 'PLACE_SETTLEMENT'

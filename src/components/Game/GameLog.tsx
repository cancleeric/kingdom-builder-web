...
function actionLabel(action: GameAction, t: TFunction<'translation'>): string {
  switch (action.type) {
    ...
    case 'TILE_PLACEMENT':
      return action.hex
        ? t('gameLog.useTileAt', {
            tile: action.tile ? t(`location.${action.tile}`) : t('gameLog.action'),
            q: action.hex.q,
            r: action.hex.r,
          })
        : t('gameLog.useTile', { tile: action.tile ? t(`location.${action.tile}`) : t('gameLog.action') });
    case 'TILE_MOVE':
      return action.fromHex && action.toHex
        ? t('gameLog.moveSettlementAt', {
              ...
              {action.acquiredTile && (
                <span className="ml-1 text-gray-500">
                  {t('gameLog.acquiredTile', {
                    ...
                    tile: t(`location.${action.acquiredTile}`),
                  })}
                </span>
              )}
            </li>

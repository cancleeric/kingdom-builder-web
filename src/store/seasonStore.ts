...
interface SeasonState {
  ...
  checkAndRotateSeason: (now?: Date) => void;
  /** Record a new score entry for the current season. */
  recordScore: (entry: LeaderboardEntry) => void;
}

// ──────────────────────────────────────────────────
// Helpers — all time logic centralised here
// ──────────────────────────────────────────────────

function currentMonthId(date: Date): string {
...
function monthEnd(date: Date): string {
  ...
  const lastDay = new Date(y, m + 1, 0); // last day of month (day 0 of next month)
...
/** Sort and find the best score + rank for the most recent entry's player name. */
...
function makeFreshSeason(now: Date): Season {
...
/** Defensive validation — prevents bad JSON from throwing. */
...
export const useSeasonStore = create<SeasonState>((set, get) => ({
  ...
  checkAndRotateSeason: (now: Date = new Date()) => {
    ...
    // Month has changed → archive old season (append then cap), start new one
  ...
  recordScore: (entry: LeaderboardEntry) => {
    const { currentSeason } = get();
    const season = currentSeason ?? makeFreshSeason(new Date());
...
// ──────────────────────────────────────────────────
// Test helper — expose store on window for CEO console testing
// ──────────────────────────────────────────────────
